...
exports.getCycleLengthDays = getCycleLengthDays;
exports.getScheduledParentForDate = getScheduledParentForDate;
exports.getNextOrdinaryHandoff = getNextOrdinaryHandoff;
const MS_PER_DAY = 24 * 60 * 60 * 1000;
/** Total längd på cykeln i dagar (t.ex. 14 för ett 2-2-3-2-2-3-mönster). */
function getCycleLengthDays(cycle) {
    let total = 0;
    for (let i = 0; i < cycle.blocks.length; i++)
        total += cycle.blocks[i].days;
    return total;
}
/**
 ...
 * faktiskt returneras.
 */
function getScheduledParentForDate(cycle, at) {
 ...
 */
function getNextOrdinaryHandoff(cycle, at) {
 ...
 */
function zonedWallClockToMs(year, monthIndex, day, hours, minutes, timeZone) {
...
function resolveCycleAnchorMs(cycle) {
